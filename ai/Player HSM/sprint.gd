...
func _update(delta: float): #runs every frame after _enter finishes
	...
	agent.velocity.x = move_toward(agent.velocity.x, agent.direction.x * agent.SPRINT, delta * 5.0)
	agent.velocity.z = move_toward(agent.velocity.z, agent.direction.z * agent.SPRINT, delta * 5.0)
