...
@export var output_var: StringName = &"target_pos" #sets/gets target node leave default in the inspector


#makes LimboAI window make more sense (renames things in the tree view)
func _generate_name() -> String:
	return "Get First Node In Group \"%s\"  ➜%s" % [group, LimboUtility.decorate_var(output_var)]
...
func _tick(_delta: float) -> Status:
	...
	if nodes.size() == 0: # if the group contains 0 nodes
		return FAILURE
	blackboard.set_var(output_var, nodes[0]) # Stores the first node in the selected group
	return SUCCESS
